  ...
  return (
    <div className={`min-h-screen bg-[#f8f7f6] text-[#3D3D3D] ${manrope.className} selection:bg-[#d7b65b]/30`}>
      ...
      <main className="flex-1 max-w-7xl mx-auto w-full px-6 md:px-10 py-12 space-y-24">
        ...
        {/* Mobile: Revenue loss banner BEFORE fold */}
        <div className="block lg:hidden -mt-6 mb-0">
          <div className="bg-[#3D3D3D] text-white rounded-2xl p-6 text-center space-y-2">
            <p className="text-sm uppercase tracking-widest text-white/50 font-bold">Est. Revenue Loss / Month</p>
            <p className="text-3xl font-black text-[#d7b65b]">{report.estRevenueLoss}</p>
            <p className="text-xs text-white/40">Based on local market data & competitor analysis</p>
          </div>
        </div>
        ...
        <section className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          ...
          {/* Score Circle — fixed overlap */}
          <div className="relative flex justify-center">
            <div className="relative w-64 h-64 md:w-80 md:h-80 lg:w-96 lg:h-96 rounded-full flex flex-col items-center justify-center shadow-2xl"
              style={{
                background: `radial-gradient(closest-side, white 79%, transparent 80% 100%), conic-gradient(#d7b65b ${progressPercent}%, #f4f3f1 0)`,
              }}>
              <span className="text-5xl md:text-7xl lg:text-8xl font-black">{report.visibilityScore}<span className="text-xl md:text-2xl lg:text-3xl text-[#3D3D3D]/30">/100</span></span>
              <span className="text-xs md:text-sm font-bold text-[#d7b65b] tracking-widest uppercase mt-2">Visibility Score</span>
            </div>
            {/* Critical Insight — positioned BELOW the circle on mobile, offset on desktop */}
            <div className="absolute -bottom-24 left-1/2 -translate-x-1/2 lg:translate-x-0 lg:left-auto lg:-bottom-4 lg:-right-4 bg-white p-5 md:p-6 rounded-2xl shadow-xl border border-[#e4e2dd] max-w-[280px] md:max-w-xs z-10">
              ...
              <p className="text-xs md:text-sm text-[#3D3D3D]/60 leading-snug">{report.criticalInsight}</p>
            </div>
          </div>
          {/* Spacer for mobile so critical insight doesn't overlap next section */}
        ...
        <section className="bg-white rounded-3xl p-8 md:p-12 shadow-sm border border-[#e4e2dd] flex flex-col md:flex-row gap-12 items-center">
          <div className="w-full md:w-1/3 aspect-square bg-gradient-to-br from-[#d7b65b]/10 to-[#d7b65b]/5 rounded-2xl flex items-center justify-center">
            <div className="text-center space-y-3">
              <p className="text-6xl md:text-7xl font-black text-[#d7b65b]">{report.visibilityScore}</p>
              <p className="text-xs font-bold uppercase tracking-widest text-[#3D3D3D]/40">Out of 100</p>
              <div className="w-16 h-1 bg-red-400 rounded-full mx-auto"></div>
              <p className="text-xs text-red-500 font-bold uppercase">Needs Attention</p>
            </div>
          </div>
        ...
        {/* Benchmark — horizontally scrollable on mobile */}
        <section className="space-y-8">
          <h2 className="text-3xl font-extrabold tracking-tight text-center">Market Benchmark</h2>
          <div className="overflow-x-auto bg-white rounded-3xl border border-[#e4e2dd] shadow-sm">
            <table className="w-full text-left border-collapse min-w-[500px]">
              ...
              <tbody className="divide-y divide-[#e4e2dd]">
                {report.benchmark.map((row, i) => (
                  <tr key={i}>
                    <td className="p-4 md:p-6 font-semibold text-sm md:text-base">{row.feature}</td>
        ...
        {/* Psychological Close — spanner icons instead of blue circles */}
          ...
          <div className="pt-4">
            <div className="flex items-center justify-center gap-3">
              <div className="flex -space-x-2">
                {/* Spanner/wrench icons in white circles */}
